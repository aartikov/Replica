...
import me.aartikov.replica.single.ReplicaEvent as Event
import me.aartikov.replica.single.ReplicaState as State
...
internal class ReplicaReducer<T : Any> : Reducer<State<T>, Action<T>, Effect<T>> {
    ...
    private fun reduceClearAction(
        ...
        ClearAction.Clear -> {
            next(
                state = state.copy(data = null, error = null),
                Effect.CancelLoading,
                Effect.EmitEvent(ClearedEvent)
            )
        }
